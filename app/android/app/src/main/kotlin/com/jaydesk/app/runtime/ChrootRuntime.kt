...
class ChrootRuntime(private val context: Context) {
    ...
    private fun configureChrootRootfs() {
        ...
        File(rootfsDir, "etc/profile.d/jaydesk-ha.sh").apply {
            parentFile?.mkdirs()
            writeText(
                """
                #!/bin/bash
                # JayDesk portable graphics environment
                export DISPLAY=:0
                ...
                # Prompt
                export PS1='\[\033[01;32m\]jaydesk\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$ '
                """.trimIndent()
            )
        }
        ...
        File(rootfsDir, "etc/apt/apt.conf.d/99-jaydesk-reliability").writeText(
            "Acquire::Retries \"3\";\n" +
    ...
    fun startSession(desktopEnv: String = "xfce4", width: Int = 1920, height: Int = 1080) {
        ...
        if (desktopEnv == "xfce4") {
            XfceMobileProfile.install(
                ...
                wallpaperFile = File(
                    rootfsDir,
                    "usr/share/backgrounds/jaydesk/ubuntu-touch.jpg",
                ),
        ...
        val runScript = """
            ...
            # Source JayDesk environment
            . /etc/profile.d/jaydesk-ha.sh 2>/dev/null || true
    ...
    /**
     * Unmount all JayDesk-related mounts.
     */
    fun unmountAll() {
        if (!hasRoot()) return
        val mounts = rootShell.exec("mount").lines()
