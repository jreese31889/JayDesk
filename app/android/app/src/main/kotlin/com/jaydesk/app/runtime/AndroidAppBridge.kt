...
object AndroidAppBridge {
    ...
    private const val SOCKET_NAME = "jaydesk.android-app-launcher"
    ...
    fun syncLaunchers(context: Context, homeDir: File, python: File, sessionRoot: File? = null) {
        ...
        val appsDir = File(homeDir, ".local/share/applications/jaydesk-android").apply { mkdirs() }
        appsDir.listFiles()?.forEach { it.delete() }
        val iconsDir = File(homeDir, ".local/share/icons/jaydesk-android").apply { mkdirs() }
        iconsDir.listFiles()?.forEach { it.delete() }

        val launcher = File(homeDir, ".local/bin/jaydesk-launch-android-app.py")
    ...
    private fun syncUtilityLaunchers(
        ...
        folders.forEach { (id, label, path) ->
            ...
            File(appsDir, "jaydesk-folder-$id.desktop").writeText(
                """
                [Desktop Entry]
                Type=Application
                Name=$label
        ...
        listOf(
            ...
            File(appsDir, "jaydesk-setting-$id.desktop").writeText(
                """
                [Desktop Entry]
                Type=Application
                Name=$label
    ...
    private fun syncDockLaunchers(
        ...
        val dockEntries = getDockPackages(context).mapIndexedNotNull { index, packageName ->
            ...
            val dockFile = File(dockDir, "jaydesk-android-$safeName.desktop")
        ...
        val idStart = "<!-- JayDesk Android dock ids start -->"
        val idEnd = "<!-- JayDesk Android dock ids end -->"
        val pluginStart = "<!-- JayDesk Android dock plugins start -->"
        val pluginEnd = "<!-- JayDesk Android dock plugins end -->"
    ...
    fun xfceDockCommand(context: Context): String {
        val dock = getDockPackages(context).mapIndexed { index, packageName ->
            val safeName = packageName.replace(Regex("[^A-Za-z0-9_.-]"), "_")
            (30 + index) to "jaydesk-android-$safeName.desktop"
        }
