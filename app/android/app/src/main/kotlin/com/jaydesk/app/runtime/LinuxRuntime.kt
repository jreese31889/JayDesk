package com.jaydesk.app.runtime
 ...
 * and Unix socket connections from /data/data/com.termux/files/usr to JayDesk's prefix.
 */
class LinuxRuntime(private val context: Context) {
    ...
    fun extractBootstrapIfNeeded(context: Context) {
        ...
        marker.writeText("JayDesk native bootstrap\n")
        tmpZip.delete()
    ...
    private fun createAptConfigOverride() {
        try {
            ...
            aptConfDir.mkdirs()
            val confFile = File(aptConfDir, "99-jaydesk-paths.conf")
            confFile.writeText(
                """
    ...
    private fun wrapDpkgForPath() {
        try {
            ...
            val relocateShebangs = File(prefixDir, "bin/jaydesk-relocate-shebangs")
    ...
    private fun wrapUpdateAlternatives() {
        try {
            ...
            uaBin.writeText(
                """
                #!/system/bin/sh
                # update-alternatives is not needed for JayDesk's single-prefix
    ...
    private fun getTermuxEnv(): Map<String, String> {
        ...
        env["APT_CONFIG"] = "${prefixDir.absolutePath}/etc/apt/apt.conf.d/99-jaydesk-paths.conf"
    ...
    fun installDesktopEnvironmentNative(
        ...
        // JayDesk embeds the X server, so termux-x11-nightly is deliberately
    ...
    fun removeStorePackage(
        ...
        if (!isSafePackageName(packageName) || isProtectedPackage(packageName)) {
            onProgress(-1.0, "This package is required by JayDesk")
            return false
        }
    ...
    fun startSession(desktopEnv: String = "xfce4", mode: String = "x11", width: Int = 1920, height: Int = 1080) {
        ...
        if (selectedDesktop == "xfce4") {
            XfceMobileProfile.install(
                ...
                wallpaperFile = File(
                    homeDir,
                    ".local/share/backgrounds/jaydesk-ubuntu-touch.jpg",
                ),
            )
        ...
        val desktopCommand = when (selectedDesktop) {
            ...
            // XFCE components and preserves JayDesk's prepared environment.
            else -> "xfce4-session"
        }
        ...
        val runScript = """
            ...
            # Use the session bus JayDesk already started
