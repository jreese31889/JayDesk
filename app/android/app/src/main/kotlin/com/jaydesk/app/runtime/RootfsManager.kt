 ...
 * official or community mirrors. They are used only by JayDesk's rooted
...
class RootfsManager(private val context: Context) {
    ...
    private fun configureRootfs() {
        ...
        // Hostname
        File(rootfsDir, "etc/hostname").writeText("jaydesk\n")
        ...
        File(rootfsDir, "etc/profile.d/jaydesk.sh").apply {
            parentFile?.mkdirs()
            writeText("""
                #!/bin/bash
                # JayDesk environment configuration
                export DISPLAY=:0
        ...
        File(rootfsDir, "etc/sudoers.d/jaydesk").apply {
            parentFile?.mkdirs()
            writeText("""
                Defaults !requiretty
